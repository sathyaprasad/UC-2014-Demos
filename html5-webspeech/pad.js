...
// default web maps in case user's groups don't return anything
 ...
 require([
     ...
     ready(function() {
        ...
        var connectStatusBtn = dojo.byId('connect-status');
        on(connectStatusBtn, clickEvent, function() {
            if(canTalk) {
         ...
         // if the screen is too small to show the title, we will show it when they click on the esri logo.
         var queryMatch = window.matchMedia('(max-width: 700px)');
         if(queryMatch.matches) {
            dojo.byId('logo').onclick = function() {
                logoClicked = !logoClicked;
                if(logoClicked) {
                    dojo.byId('title').style.display = 'inline-block';
                }                
                else {
                    dojo.byId('title').style.display = 'none';
                }
            };
         }

         // see if supports device orientation (i.e. gyro)
         if(window.DeviceOrientationEvent) {
            window.addEventListener('deviceorientation', onDeviceOrientationChanged);
         }  
         else {
            alert('Your browser doesn\'t support device orientation. Sorry!');
         }
         ...
         if (!('webkitSpeechRecognition' in window)) {
             ...
             // brings back a list of most likely results
 ...
 // depending on the command, do something to the map or page.
